...
let badwordRegex = /anj(k|g)|ajn?(g|k)|a?njin(g|k)|bajingan|b(a?n)?gsa?t|ko?nto?l|me?me?(k|q)|pe?pe?(k|q)|meki|titi(t|d)|pe?ler|tetek|toket|ngewe|go?blo?k|to?lo?l|idiot|(k|ng)e?nto?(t|d)|jembut|bego|dajj?al|janc(u|o)k|pantek|puki ?(mak)?|kimak|kampang|lonte|col(i|mek?)|whore|henceu?t|nigga|fuck|dick|bitch|tits|bastard|asshole/i // tambahin sendiri
...
"Never use a big word when a little filthy one will do" (ʀᴏꜱᴇ-ᴍᴡᴏʟ).`, watermark, 'Turn on Antibadword', ',1 antibadword',  'sorry', m)
        if (user.warning >= 5) {
            user.banned = true
            if (m.isGroup) {
                if (isBotAdmin) {
