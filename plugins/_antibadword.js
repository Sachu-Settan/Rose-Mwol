...
Warning: ${user.warning} / 100
If the warning reaches 100 you will be banned

type *#on antibadword* to turn on antibadword
type *#sorry* to reduce the warning

"Never use a big word when a little filthy one will do" (Rose).`, watermark, 'Turn on Antibadword', ',1 antibadword',  'sorry', m)
        if (user.warning >= 100) {
            user.banned = true
            if (m.isGroup) {
                if (isBotAdmin) {
